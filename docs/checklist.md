...
- [x] #29 [Feat] 선택 곡 기준 최근접/최원거리 곡 강조 구현
  <details>
  <summary>🔍 작업 내용 보기</summary>
...
## 📋 세부 작업 태스크 (Todo)
- [x] 거리 계산 기준 선택: 원본 5D 벡터 우선, 필요 시 3D 투영 좌표 fallback
- [x] selectedTrackId 기준 nearest/farthest 계산 유틸 구현
- [x] 최근접/최원거리 노드 색상 또는 emissive intensity 차별화
- [x] 선택 곡과 최근접/최원거리 곡을 잇는 R3F line 렌더링
- [x] HUD 또는 hover card에 관계 라벨 표시
- [x] 선택 곡이 없거나 노드가 2개 미만일 때 fallback 처리

## ✅ 검증 방법
- [x] 선택 곡 변경 시 최근접/최원거리 강조 대상이 갱신되는지 확인
- [x] 연결선이 올바른 두 노드를 잇는지 확인
- [x] 250개 노드에서도 조작감이 유지되는지 확인
- [x] `corepack pnpm run test`
- [x] `corepack pnpm run lint`
- [x] `corepack pnpm run build`
...
- [x] #30 [Feat] 선택 곡 변경 시 관계 재연산 흐름 구현
  <details>
  <summary>🔍 작업 내용 보기</summary>
...
## 📋 세부 작업 태스크 (Todo)
- [x] selectedTrackId를 단일 source of truth로 정리
- [x] 선택 곡 변경 시 관계 계산 결과를 memoized selector로 재계산
- [x] 선택 상태가 공유 snapshot URL에 반영되는지 확인
- [x] 선택 곡 기준 관계 UI/HUD/line이 동시에 갱신되도록 연결
- [x] 선택 곡이 snapshot URL로 복원된 경우에도 동일 계산이 수행되도록 처리
- [x] 불필요한 전체 노드 재생성 없이 계산/렌더링이 갱신되도록 최적화

## ✅ 검증 방법
- [x] 여러 곡을 연속 선택해도 관계 강조가 이전 상태에 남지 않는지 확인
- [x] 선택 곡이 포함된 공유 URL을 새로 열었을 때 관계가 즉시 복원되는지 확인
- [x] `corepack pnpm run test`
- [x] `corepack pnpm run lint`
- [x] `corepack pnpm run build`

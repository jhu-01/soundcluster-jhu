...
## 📋 세부 작업 태스크 (Todo)
- [x] `client/src/components/SearchBar.tsx` iTunes 검색어 입력 및 검색 요청 바인딩
...
- [x] #33 [Feat] nanoid 기반 짧은 공유 URL 저장 흐름 구현
  <details>
...
- [x] `POST /api/share-snapshots`로 snapshot 저장 및 share id 반환
- [x] `GET /api/share-snapshots/:shareId`로 snapshot 복원
- [x] 공유 모달에서 짧은 share URL 생성
- [x] 앱 진입 시 `?share=` query를 읽어 snapshot 복원
- [x] 기존 `snapshot=` compact URL 복원 호환 유지
...
- [x] 실제 `POST /api/share-snapshots` 및 `GET /api/share-snapshots/:shareId` 저장/조회 검증
...
## 📋 세부 작업 태스크 (Todo)
- [x] `client/src/components/SearchBar.tsx`에서 곡명 또는 아티스트 검색어 입력 상태 관리
...
## 📋 세부 작업 태스크 (Todo)
- [x] `client/src/components/SearchBar.tsx` 곡명 또는 아티스트 검색 UI 구현
- [x] `client/src/components/ControlPanel.tsx` 5축 축 선택 토글 구현
- [x] 내부 분석 로그 UI를 제거하고 사용자-facing HUD만 유지
- [x] 선택 곡 정보와 감성 축 토글을 사용자에게 표시
...
- [x] #32 [Feat] LRCLIB 기반 가사 조회 및 Gemini 분석 입력 연동
  <details>
  <summary>작업 내용 보기</summary>

  ## 작업 개요
현재 iTunes API는 곡명, 아티스트, 앨범 이미지 등 메타데이터만 제공하고 가사는 제공하지 않습니다. Gemini 감성 분석 품질을 높이기 위해 LRCLIB API를 서버에서 호출하여 실제 가사를 조회하고, 조회된 lyrics를 `/api/analyze/stream` 요청에 포함해 LLM 분석 입력으로 사용합니다.

## 세부 작업 태스크 (Todo)
- [x] 서버에 LRCLIB lyrics route/service 추가
- [x] `GET /api/lyrics/search?title=...&artist=...` API 설계
- [x] LRCLIB `/api/search`에서 `plainLyrics` 우선 사용
- [x] `plainLyrics`가 없고 `syncedLyrics`만 있으면 타임코드 제거 후 사용
- [x] lyrics가 없으면 title/artist 기반 분석으로 fallback
- [x] Gemini 분석 요청에서 dummy fallback lyrics 제거
- [x] iTunes `+` 클릭 흐름을 `metadata -> lyrics lookup -> Gemini stream -> 3D render`로 연결
- [x] debug panel에 iTunes response, LRCLIB response, Gemini analysis response를 구분 표시
- [x] lyrics 조회 실패가 전체 extract 흐름을 깨지 않도록 error/fallback 처리

## 검증 방법
- [x] 실제 곡명/아티스트로 LRCLIB lyrics response 확인
- [x] lyrics가 있는 곡은 Gemini request에 lyrics가 포함되는지 확인
- [x] lyrics가 없는 곡도 title/artist 기반으로 분석이 진행되는지 확인
- [x] debug panel에서 iTunes/LRCLIB/Gemini response가 구분되어 표시되는지 확인
- [x] `corepack pnpm run lint`
- [x] `corepack pnpm run test`
- [x] `corepack pnpm run build`
- [x] `corepack pnpm run server:build`

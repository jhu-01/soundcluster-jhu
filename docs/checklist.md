...
- [ ] #15 [Feat] EventSource 스트림 상태 훅 및 전역 분석 상태 연결
  <details>
...
## 📋 세부 작업 태스크 (Todo)
- [ ] `client/src/hooks/useEmotionStream.ts`에서 `EventSource` 연결/수신/에러/종료 처리
- [ ] `client/src/context/AnalysisContext.tsx` 또는 동등한 상태 계층 생성
- [ ] `AnalyzeStreamEvent`와 최종 `MusicAnalysisResponse`를 상태에 저장
- [ ] 중복 연결 방지와 컴포넌트 unmount 시 `stream.close()` 처리

## ✅ 검증 방법
- [ ] 백엔드: `corepack pnpm run server:build`
- [ ] 프론트엔드: `corepack pnpm run build`
- [ ] 브라우저에서 스트림 시작/완료/재시작 시 상태가 누적 오류 없이 갱신되는지 확인

  </details>

- [ ] #16 [Feat] 검색 입력값을 SSE 분석 query payload로 연결
  <details>
  <summary>🔍 작업 내용 보기</summary>
...
## 📋 세부 작업 태스크 (Todo)
- [ ] `client/src/components/SearchBar.tsx`에서 곡명/아티스트/가사 입력 상태 관리
- [ ] `URLSearchParams`로 EventSource URL 생성
- [ ] 같은 곡명/아티스트 요청 시 DB cache hit가 즉시 반환되는지 FE에서 확인
- [ ] 빈 입력, 긴 가사, 특수문자 입력에 대한 최소 validation 적용

## ✅ 검증 방법
- [ ] `curl.exe -N "http://127.0.0.1:3001/api/analyze/stream?title=...&artist=..."`
- [ ] 브라우저에서 동일 입력 2회 요청 시 두 번째 요청이 즉시 완료되는지 확인

  </details>

- [ ] #17 [Feat] 5차원 감성 벡터를 3D 좌표로 변환하는 매핑 유틸 구현
  <details>
  <summary>🔍 작업 내용 보기</summary>
...
## 📋 세부 작업 태스크 (Todo)
- [ ] `shared/utils/emotionToPoint.ts` 또는 `client/src/utils/mds.ts`에 순수 매핑 함수 작성
- [ ] 5차원 값으로 `{ position, color, scale, intensity }` 생성
- [ ] 목데이터와 실제 분석 결과가 같은 매핑 함수를 쓰도록 `StarsCanvas.tsx` 정리
- [ ] 좌표 범위, 스케일링, 색상 매핑 상수 분리

## ✅ 검증 방법
- [ ] `corepack pnpm run build`
- [ ] `corepack pnpm run lint`
- [ ] 여러 감성 벡터가 겹치지 않고 화면 중앙 주변에 안정적으로 분포하는지 확인

  </details>

- [ ] #18 [Feat] SSE visual payload 기반 R3F 진행 애니메이션 연결
  <details>
  <summary>🔍 작업 내용 보기</summary>
...
## 📋 세부 작업 태스크 (Todo)
- [ ] 스트리밍 진행 중 `activeNodeCount`에 따라 노드가 순차 등장
- [ ] `visual.intensity`를 emissive intensity와 bloom 후보 값에 반영
- [ ] `visual.orbitSpeed`를 노드 그룹 회전 속도에 반영
- [ ] `done` 수신 시 최종 분석 벡터 기반 위치로 부드럽게 안착

## ✅ 검증 방법
- [ ] `corepack pnpm run build`
- [ ] 브라우저에서 분석 시작 후 노드가 단계적으로 나타나고 완료 시 최종 벡터 위치에 고정되는지 확인

  </details>

- [ ] #19 [Feat] R3F 노드 컴포넌트 분리 및 인터랙션 구현
  <details>
  <summary>🔍 작업 내용 보기</summary>
...
## 📋 세부 작업 태스크 (Todo)
- [ ] `client/src/canvas/StarNode.tsx` 개별 별 노드 구현
- [ ] `client/src/canvas/StarNodeCollection.tsx` 노드 목록 렌더링 구현
- [ ] hover 시 크기/색상/밝기 변화
- [ ] click 시 선택된 곡 정보를 HUD 또는 상세 패널로 전달할 수 있는 callback 추가

## ✅ 검증 방법
- [ ] `corepack pnpm run build`
- [ ] 브라우저에서 hover/click 반응 확인

  </details>

- [ ] #20 [Feat] 분석 HUD와 5축 감성 제어 패널 완성
  <details>
  <summary>🔍 작업 내용 보기</summary>

  ## 🎯 작업 개요
R3F 캔버스 위에 검색 입력, 분석 버튼, 진행률, 5축 감성 슬라이더, 최종 요약을 표시하는 HUD를 구현합니다.

## 📋 세부 작업 태스크 (Todo)
- [ ] `client/src/components/SearchBar.tsx` 곡명/아티스트/가사 입력 UI 구현
- [ ] `client/src/components/ControlPanel.tsx` 5축 슬라이더 구현
- [ ] `client/src/components/StreamingLogViewer.tsx` 진행 상태를 압축적으로 표시
- [ ] 최종 `generatedSummary`와 cache hit 여부를 사용자에게 표시

## ✅ 검증 방법
- [ ] 모바일/데스크톱에서 HUD가 캔버스 조작을 과도하게 가리지 않는지 확인
- [ ] 빈 입력/분석 중 중복 클릭/완료 후 재분석 플로우 확인
...
- [ ] #26 [Feat] Spotify API 기반 곡 메타데이터 조회 구현
  <details>
  <summary>🔍 작업 내용 보기</summary>

  ## 🎯 작업 개요
곡명/가수 입력만으로 Spotify API에서 실제 곡 메타데이터를 조회해 곡명, 가수, 앨범 이미지를 확보하고, 이후 Gemini 분석 및 3D 노드 표시에서 동일한 메타데이터를 사용합니다.

## 📋 세부 작업 태스크 (Todo)
- [ ] Spotify API 인증 방식 선택 및 환경 변수 설계
- [ ] 서버 측 Spotify client credentials token 발급/갱신 유틸 구현
- [ ] 곡명/가수 기반 track search API wrapper 구현
- [ ] 검색 결과에서 title, artist, albumImageUrl, spotifyTrackId 정규화
- [ ] 프론트 검색 결과 또는 분석 요청 흐름에 Spotify 메타데이터 바인딩
- [ ] 앨범 이미지가 있는 경우 노드/HUD/공유 snapshot에서 사용할 필드 구조 확장
- [ ] Spotify 실패 또는 결과 없음 fallback 처리

## ✅ 검증 방법
- [ ] 실제 곡명/가수로 Spotify 검색 결과가 반환되는지 확인
- [ ] 앨범 이미지 URL이 프론트에서 표시 가능한지 확인
- [ ] Spotify 결과 없음/인증 실패 시 기존 수동 입력 흐름이 깨지지 않는지 확인
- [ ] `corepack pnpm run lint`
- [ ] `corepack pnpm run build`

  </details>

- [ ] #27 [Perf] 공유 snapshot URL 압축 인코딩 구현
  <details>
  <summary>🔍 작업 내용 보기</summary>
...
## 📋 세부 작업 태스크 (Todo)
- [ ] snapshot payload에서 반복 key를 줄이는 compact schema 설계
- [ ] `version` 기반으로 기존 base64 JSON snapshot과 새 압축 snapshot을 모두 decode 가능하게 유지
- [ ] 카메라, 선택 노드, 트랙 id/title/artist/emotion vector를 짧은 배열 구조로 직렬화
- [ ] emotion float 값을 필요한 소수점 정밀도로 quantize해 URL 길이 절감
- [ ] `createShareSnapshotUrl()`이 새 압축 encoder를 사용하도록 변경
- [ ] 잘못된 snapshot query fallback 처리 유지

## ✅ 검증 방법
- [ ] 같은 snapshot 기준 기존 URL 대비 길이 감소 확인
- [ ] 압축 URL을 새 탭에서 열면 동일한 선택 노드/카메라/트랙 값이 복원되는지 확인
- [ ] 기존 base64 JSON snapshot URL도 계속 복원되는지 확인
...
- [ ] #28 [Feat] 3D 노드 hover 메타데이터 카드 구현
  <details>
  <summary>🔍 작업 내용 보기</summary>

  ## 🎯 작업 개요
3D 렌더링 상태에서 곡 노드에 hover하면 Spotify API로 확보한 곡명, 아티스트, 앨범 이미지를 화면에 표시합니다.

## 📋 세부 작업 태스크 (Todo)
- [ ] shared track/snapshot 타입에 title, artist, albumImageUrl, spotifyTrackId 필드 정리
- [ ] R3F 노드 hover 상태를 App 또는 HUD 레이어로 전달
- [ ] `TrackHoverCard` 또는 동등한 오버레이 컴포넌트 구현
- [ ] hover 카드에 곡명, 아티스트, 앨범 이미지 표시
- [ ] 앨범 이미지가 없거나 로딩 실패할 때 fallback UI 적용
- [ ] 모바일/터치 환경에서는 hover 대신 tap/focus fallback 동작 정의

## ✅ 검증 방법
- [ ] 노드 hover 시 올바른 곡명/아티스트/앨범 이미지가 표시되는지 확인
- [ ] hover 해제 시 카드가 사라지는지 확인
- [ ] 앨범 이미지 없음/오류 상황에서 UI가 깨지지 않는지 확인
- [ ] `corepack pnpm run lint`
- [ ] `corepack pnpm run build`

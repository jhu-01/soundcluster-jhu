...
- [ ] #32 [Feat] LRCLIB 기반 가사 조회 및 Gemini 분석 입력 연동
  <details>
  <summary>작업 내용 보기</summary>
...
## 세부 작업 태스크 (Todo)
- [ ] 서버에 LRCLIB lyrics route/service 추가
- [ ] `GET /api/lyrics/search?title=...&artist=...` API 설계
- [ ] LRCLIB `/api/search`에서 `plainLyrics` 우선 사용
- [ ] `plainLyrics`가 없고 `syncedLyrics`만 있으면 타임코드 제거 후 사용
- [ ] lyrics가 없으면 title/artist 기반 분석으로 fallback
- [ ] Gemini 분석 요청에서 dummy fallback lyrics 제거
- [ ] iTunes `+` 클릭 흐름을 `metadata -> lyrics lookup -> Gemini stream -> 3D render`로 연결
- [ ] debug panel에 iTunes response, LRCLIB response, Gemini analysis response를 구분 표시
- [ ] lyrics 조회 실패가 전체 extract 흐름을 깨지 않도록 error/fallback 처리

## 검증 방법
- [ ] 실제 곡명/아티스트로 LRCLIB lyrics response 확인
- [ ] lyrics가 있는 곡은 Gemini request에 lyrics가 포함되는지 확인
- [ ] lyrics가 없는 곡도 title/artist 기반으로 분석이 진행되는지 확인
- [ ] debug panel에서 iTunes/LRCLIB/Gemini response가 구분되어 표시되는지 확인

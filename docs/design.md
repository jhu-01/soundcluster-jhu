...
- Pinned relation popups use the default popup surface without gradient borders.
- Selected, nearest, and farthest song markers are emphasized by scale only: selected around 3x, nearest and farthest around 2x.
- A small bottom-left debug panel may show the latest API response while validating search/extract behavior.
...
사용자가 상단 검색창에 곡명 또는 아티스트를 입력하면 iTunes API에서 검색 가능한 곡 후보를 좌측 패널에 보여준다. 사용자가 후보 옆의 `+` 버튼을 누르면 해당 곡 메타데이터로 LLM 분석을 요청하고, 결과로 나온 감성 축별 `0.0 ~ 1.0` 값을 3D 좌표로 변환한다. 변환된 곡은 우주 공간의 별처럼 3D 렌더링 영역에 자동 반영된다.
...
```text
┌──────────────────────────────────────────────────────────────────────────────┐
│ SoundCluster          [ Search by song title or artist... ] [ Search ]        │
│                                                                              │
│ ┌───────────────┐        ┌───────────────────────────────┐   ┌────────────┐ │
...
| Search Button | Search Bar 오른쪽 | iTunes API 검색 요청 |
...
4. 사용자가 결과 하나를 선택
5. 좌측 검색 결과의 `+` 버튼으로 해당 곡 Extract 실행
6. 클릭 시 LLM 분석 요청 시작
...
| 결과 있음 | 좌측 패널에 후보 표시 |
| 검색 결과 있음 | 각 결과 옆 `+` 버튼 표시 |
| LLM 분석 중 | 해당 결과 `+` 버튼에 loading state |
